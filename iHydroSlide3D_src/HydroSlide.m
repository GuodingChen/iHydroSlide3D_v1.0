function [cSM,channel,overland,interflow,infil,sr,FoS_inf,caET,cP,cERO,cERI,cPOT, tot_infil] = ...
    HydroSlide(rundata,cSM,Pin,PET,dt,tstep,cstep,compgrid,dx,...
    NEXTTIMEUNDER,TH,ALPHA,BETA,SLOPE_TH,tot_infil)
...
%cER - Excess Rainfall
[cI((cPSoil > 0) & (cSM < PWM)),cER((cPSoil > 0) & (cSM < PWM)),W0((cPSoil > 0) & (cSM < PWM)),cperc((cPSoil > 0) & (cSM < PWM))] = Derive_vic(cPSoil((cPSoil > 0) & (cSM < PWM)),vicPars,cSM((cPSoil > 0) & (cSM < PWM)), interflow((cPSoil > 0) & (cSM < PWM)));
...
%EF5 Implementation
ctemX(cPSoil > 0) = ((cSM(cPSoil > 0) + W0(cPSoil > 0)) ./ (2.*PWM(cPSoil > 0))) .* K(cPSoil > 0); %Calculate how much water can infiltrate
...
[channel,overland,interflow] = flowrouting(cERO.*(0.001/dt),cERI,channel,overland,NEXTTIMEUNDER,ALPHA,BETA,TH,SLOPE_TH,dt,dx,rundata);
infil=cI;
...
FoS_inf = StabilityEQ_1D(rundata.slopeDeg, rundata.Parameters.cohesion, ...
    rundata.Parameters.friction, rundata.Parameters.porosity, ...
    rundata.Parameters.hydrocond, cstep, tot_infil, sr);
